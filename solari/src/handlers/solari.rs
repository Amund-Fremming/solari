...
#[derive(Debug, Serialize)]
pub struct VippsWebhookResponse {
    pub ok: bool,
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/webhooks/vipps", post(vipps_webhook))
}
...
async fn vipps_webhook(Json(_body): Json<Value>) -> Json<VippsWebhookResponse> {
    Json(VippsWebhookResponse {
        ok: true,
        provider: "vipps",
        received: true,
    })
