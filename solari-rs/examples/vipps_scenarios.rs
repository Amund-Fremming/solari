...
fn has_required_vipps_env() -> bool {
    [
        "VIPPS_BASE_URL",
        "VIPPS_CLIENT_ID",
        "VIPPS_CLIENT_SECRET",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !has_required_vipps_env() {
        info!(
            "Skipping Vipps scenarios: set VIPPS_BASE_URL, VIPPS_CLIENT_ID, VIPPS_CLIENT_SECRET, VIPPS_SUBSCRIPTION_KEY, and VIPPS_MSN"
        );
        return Ok(());
    }
