export const VIPPS_BUTTON_SCRIPT_URL = "https://checkout.vipps.no/checkout-button/v1/vipps-checkout-button.js";
...
export function loadVippsButtonScript() {
    return new Promise((resolve, reject) => {
        // Check if script is already loaded
        if (window.VippsCheckoutButton) {
            resolve();
            return;
        }
        // Check if script tag already exists
        const existingScript = document.querySelector(`script[src="${VIPPS_BUTTON_SCRIPT_URL}"]`);
        if (existingScript) {
            existingScript.addEventListener("load", () => resolve());
            existingScript.addEventListener("error", () => reject(new Error("Failed to load Vipps button script")));
            return;
        }
        ...
        script.addEventListener("error", () => reject(new Error("Failed to load Vipps button script")));
        document.head.appendChild(script);
    });
}
export function getVippsButtonSvgUrl() {
    return new URL(VIPPS_BUTTON_SVG_PATH, import.meta.url).toString();
}
//# sourceMappingURL=VippsButton.js.map